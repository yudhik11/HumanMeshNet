...
class SMPL(nn.Module):
    def __init__(self, model_path = '../../model_smpl_male.pkl'):
...
#             print(key, "\t\t", type(model[key]))
        ...
        np_J_regressor = np.array(model['J_regressor'].toarray().T, dtype = np.float)
        self.register_buffer('J_regressor', torch.from_numpy(np_J_regressor).float())

        np_faces = np.array(model['f'], dtype = np.float)
        self.register_buffer('faces', torch.from_numpy(np_faces).float())
        ...
        self.SMPL_to_H36M_indices16 = torch.tensor([11, 5, 2, 1, 4, 10, 0, 6, 12, 12, 21, 19, 17, 16, 18, 20])
        self.SMPL_to_H36M_indices14 = torch.tensor([11, 5, 2, 1, 4, 10, 12, 12, 21, 19, 17, 16, 18, 20])
        self.head_index = 410
...
#         print("theta : ", theta.size())
        ...
        self.J_transformed, A = batch_global_rigid_transformation(Rs, J, self.parents, self.cur_device, rotate_base = False)
...
#         print("joints", joints.size())
    ...
    def H36M_forward(self, beta, theta, get_skin=False):

        verts, joints, Rs = self.forward(beta, theta, get_skin=True)
        new_joints = joints[:, self.SMPL_to_H36M_indices14, :]
        new_joints[:, 7, :] = verts[:, self.head_index, :]

        if get_skin:
            return verts, new_joints, Rs
        else:
            return new_joints

    def H36M_forward_old(self, beta, theta, get_skin=False):

        verts, joints, Rs = self.forward(beta, theta, get_skin=True)
        new_joints = joints[:, self.SMPL_to_H36M_indices16, :]
        new_joints[:, 9, :] = verts[:, self.head_index, :]
